      ...
      <ul>
        {tasks.map((task) => (
          <li key={task._id} className="border p-2 flex justify-between items-center mb-2">
            <div>
              <span className="font-bold">{task.title}</span>: {task.description} <br />
            ...
            <div>
              <button onClick={() => handleDeleteTask(task._id)} className="bg-red-500 text-white px-4 py-2 rounded">
                Delete
              </button>
            </div>
          </li>
